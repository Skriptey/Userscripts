...
album, song and music-video pages it surfaces what the web player already knows
but hides: **audio formats**, the **barcode (UPC)**, and **per-track ISRCs** —
with one-click copy and a [MagicISRC](https://magicisrc.kepstin.ca/) link.
...
- **Cover-art download** — static (highest-res JPEG); and when the album has
  **animated (motion) artwork**, a dropdown for **Static / Square / Vertical /
  All** (`.mp4`). **All** → a `<artist> - <album>_CoverArt.zip` at the L
  resolution. Resolution is set via **Animated cover-art resolution** (L 1080 /
  XL 2160 / Max highest).
- Works on **album / song / music-video** pages, every storefront, and follows
  Apple Music's single-page navigation.
