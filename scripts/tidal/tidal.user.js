...
(function () {
  ...
  /** Find the album-title heading (robust to Tidal's hashed class names): the
  ...
  /** Place the inline header UI, RETRYING UNTIL IT LANDS. Tidal renders the album
   *  header at very different times across pages — ones with many tracks or
   *  credits render notably later — so the old fixed ~5s retry window left the
   *  badges/buttons missing on those (while the on-demand panel always worked). A
  ...
  function placeHeaderUI() {
    ...
    const page = parsePage();
    const placed = () =>
      document.querySelector('.tde-actions') || document.querySelector('.tde-badges');
    const deadline = Date.now() + 15000; // overall cap (mostly waiting for render)
