...
(function () {
  ...
  /** Find the album/track-title heading (robust to Spotify's hashed class names):
   *  the heading whose text matches the entity name, else the first non-empty
   *  heading near the top. Scoped to <main> so a nav/sidebar/shelf heading can't
   *  win; case-insensitive; prefers a STARTS-WITH match, then a CONTAINS match,
   *  with a main-<h1> fallback. */
  function findTitleAnchor(name) {
    ...
    // Otherwise the title is the main <h1>; fall back to any non-empty heading.
  ...
  /** Place the inline header UI, RETRYING UNTIL IT LANDS. Spotify renders the
   *  album/track header (and the player mints the token we sniff) at very
   *  different times across pages — ones with long tracklists render notably
   *  later — so the old fixed retry window left the buttons missing on those
   *  (while the on-demand panel always worked). A MutationObserver plus a bounded
   *  poll keep trying until placed, or ~15s. fetchEntity is cached and
  ...
  function placeHeaderUI() {
    ...
    const page = parsePage();
    const placed = () => document.querySelector('.spe-actions');
